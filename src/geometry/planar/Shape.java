...
/**
 * Interface describing functionality for connected 2-dimensional shapes in the
 * plane. A Shape object is expected to be simply connected, that means, it may
 * not contain holes.
 *
 * @author Alfons Wirtz
 */
public interface Shape extends Area {
    ...
    /**
     * Returns the length of the border of this shape. If the shape is
    ...
    /**
     * Returns true, if p_point is not contained in the inside or the boundary
     * of the shape
     */
    boolean is_outside(Point p_point);

    /**
     * Returns true, if p_point is contained in this shape, but not on the
     * border.
     */
    ...
    /**
     * Returns the distance between p_point and its nearest point on the border
     * of the shape.
     */
    double border_distance(FloatPoint p_point);

    /**
     * Returns the smallest distance from the centre of gravity to the border of
     * the shape.
     */
    double smallest_radius();

    /**
     * Returns the offset shape of this shape by offseting the boundary by
     * p_distance to the outside. The result instsnce may be of a different
     * class than this instance. (For example an enlarged IntBox is an
     * IntOctagon).
     */
