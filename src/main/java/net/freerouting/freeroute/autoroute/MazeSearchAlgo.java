...
import net.freerouting.freeroute.board.ForcedViaAlgo;
...
import net.freerouting.freeroute.geometry.planar.FloatLine;
...
public class MazeSearchAlgo {
    ...
    private static void reduce_trace_shapes_at_tie_pins(Collection<Item> p_item_list, int p_own_net_no, ShapeSearchTree p_autoroute_tree) {
        for (Item curr_item : p_item_list) {
            if ((curr_item instanceof net.freerouting.freeroute.board.Pin) && curr_item.net_count() > 1) {
                Collection<Item> pin_contacts = curr_item.get_normal_contacts();
                net.freerouting.freeroute.board.Pin curr_tie_pin = (net.freerouting.freeroute.board.Pin) curr_item;
                for (Item curr_contact : pin_contacts) {
                    if (!(curr_contact instanceof net.freerouting.freeroute.board.PolylineTrace) || curr_contact.contains_net(p_own_net_no)) {
                        continue;
                    }
                    p_autoroute_tree.reduce_trace_shape_at_tie_pin(curr_tie_pin, (net.freerouting.freeroute.board.PolylineTrace) curr_contact);
                }
            }
        }
    }
    ...
    private static double calc_fanout_via_ripup_cost_factor(net.freerouting.freeroute.board.Trace p_trace) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (curr_trace_contact instanceof net.freerouting.freeroute.board.Pin && curr_trace_contact.first_layer() == curr_trace_contact.last_layer()) {
                protect_fanout_via = true;
            } else if (curr_trace_contact instanceof PolylineTrace && curr_trace_contact.get_fixed_state() == net.freerouting.freeroute.board.FixedState.SHOVE_FIXED) {
    ...
    private boolean expand_to_room_doors(MazeListElement p_list_element) {
        ...
        if (this.ctrl.with_neckdown && p_list_element.door instanceof TargetItemExpansionDoor) {
            ...
            if (start_item instanceof net.freerouting.freeroute.board.Pin) {
                double neckdown_half_width = ((net.freerouting.freeroute.board.Pin) start_item).get_trace_neckdown_halfwidth(layer_no);
        ...
        if (p_list_element.next_room instanceof FreeSpaceExpansionRoom) {
            ...
            if (!p_list_element.already_checked) {
                ...
                if (ripup_costs != ALREADY_RIPPED_COSTS && next_room_is_thick) {
                    ...
                    if (!curr_door_is_small && this.ctrl.max_shove_trace_recursion_depth > 0 && obstacle_item instanceof net.freerouting.freeroute.board.PolylineTrace) {
        ...
        if (ctrl.vias_allowed && !(p_list_element.door instanceof ExpansionDrill)) {
            if ((something_expanded || next_room_is_thick) && p_list_element.next_room instanceof CompleteFreeSpaceExpansionRoom) {
                ...
                if (curr_obstacle_item instanceof net.freerouting.freeroute.board.Via) {
                    net.freerouting.freeroute.board.Via curr_via = (net.freerouting.freeroute.board.Via) curr_obstacle_item;
    ...
    private boolean door_is_small(ExpansionDoor p_door, double p_trace_width) {
        if (p_door.dimension == 1 || p_door.first_room instanceof CompleteFreeSpaceExpansionRoom && p_door.second_room instanceof CompleteFreeSpaceExpansionRoom) {
            ...
            if (door_shape.is_empty()) {
                if (this.autoroute_engine.board.get_test_level().ordinal() >= net.freerouting.freeroute.board.TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
    ...
    private void expand_to_drill(ExpansionDrill p_drill, MazeListElement p_from_element, int p_add_costs) {
        ...
        if (p_from_element.door instanceof DrillPage && p_from_element.backtrack_door instanceof TargetItemExpansionDoor) {
            ...
            if (from_item instanceof net.freerouting.freeroute.board.Pin) {
                FloatPoint nearest_exit_corner = ((net.freerouting.freeroute.board.Pin) from_item).nearest_trace_exit_corner(p_drill.location.to_float(), trace_half_width, layer);
    ...
    private void expand_to_other_layers(MazeListElement p_list_element) {
        ...
        if (curr_drill.room_arr[p_list_element.section_no_of_door] instanceof ObstacleExpansionRoom) {
            ...
            for (;;) {
                ...
                net.freerouting.freeroute.board.ForcedPadAlgo.CheckDrillResult drill_result
                ...
                if (drill_result == net.freerouting.freeroute.board.ForcedPadAlgo.CheckDrillResult.NOT_DRILLABLE) {
                    via_lower_bound = curr_layer + 1;
                    break;
                } else if (drill_result == net.freerouting.freeroute.board.ForcedPadAlgo.CheckDrillResult.DRILLABLE_WITH_ATTACH_SMD) {
                    if (curr_layer == 0) {
    ...
    private boolean room_shape_is_thick(ObstacleExpansionRoom p_obstacle_room) {
        ...
        if (obstacle_item instanceof net.freerouting.freeroute.board.Trace) {
            obstacle_half_width = ((net.freerouting.freeroute.board.Trace) obstacle_item).get_half_width()
    ...
    private int check_ripup(MazeListElement p_list_element, Item p_obstacle_item, boolean p_door_is_small) {
        ...
        if (p_obstacle_item instanceof net.freerouting.freeroute.board.Trace) {
            net.freerouting.freeroute.board.Trace obstacle_trace = (net.freerouting.freeroute.board.Trace) p_obstacle_item;
        ...
        } else if (p_obstacle_item instanceof net.freerouting.freeroute.board.Via) {
            ...
            for (Item curr_contact : contact_list) {
                if (!(curr_contact instanceof net.freerouting.freeroute.board.Trace) || curr_contact.is_user_fixed()) {
                    return -1;
                }
                ++contact_count;
                net.freerouting.freeroute.board.Trace obstacle_trace = (net.freerouting.freeroute.board.Trace) curr_contact;
    ...
    private double check_neck_down_at_dest_pin(CompleteExpansionRoom p_room) {
        ...
        for (TargetItemExpansionDoor curr_target_door : target_doors) {
            if (curr_target_door.item instanceof net.freerouting.freeroute.board.Pin) {
                return ((net.freerouting.freeroute.board.Pin) curr_target_door.item).get_trace_neckdown_halfwidth(p_room.get_layer());
            }
        }
        return 0;
    }
