...
class DrillPage implements ExpandableObject {
    ...
    public Collection<ExpansionDrill> get_drills(AutorouteEngine p_autoroute_engine, boolean p_attach_smd) {
        if (this.drills == null || p_autoroute_engine.get_net_no() != this.net_no) {
            ...
            for (TreeEntry curr_entry : overlaps) {
                ...
                if (curr_item instanceof net.freerouting.freeroute.board.Pin) {
                    if (p_attach_smd && ((net.freerouting.freeroute.board.Pin) curr_item).drill_allowed()) {
                        continue;
                    }
                }
    ...
    public void draw(java.awt.Graphics p_graphics,
            net.freerouting.freeroute.boardgraphics.GraphicsContext p_graphics_context, double p_intensity) {
        if (true || drills == null) {
            return;
        }
