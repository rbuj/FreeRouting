...
import net.freerouting.freeroute.board.ShoveTraceAlgo;
...
import net.freerouting.freeroute.geometry.planar.LineSegment;
...
public class MazeShoveTraceAlgo {
    ...
    public static boolean check_shove_trace_line(MazeListElement p_list_element,
        ...
        if (!(p_obstacle_room.get_item() instanceof net.freerouting.freeroute.board.PolylineTrace)) {
            return true;
        }
        net.freerouting.freeroute.board.PolylineTrace obstacle_trace = (net.freerouting.freeroute.board.PolylineTrace) p_obstacle_room.get_item();
        ...
        for (ExpansionDoor curr_door : room_doors) {
            ...
            if (curr_door.dimension == 2 && shove_width >= Integer.MAX_VALUE) {
                ...
                if (curr_door_segment == null) {
                    if (p_board.get_test_level() == net.freerouting.freeroute.board.TestLevel.ALL_DEBUGGING_OUTPUT) {
                ...
                } else if (start_corner_side_of_trace_line != Side.ON_THE_RIGHT || end_corner_side_of_trace_line != Side.ON_THE_RIGHT) {
                    continue;
                }
     ...
     */
    private static boolean end_points_matching(net.freerouting.freeroute.board.PolylineTrace p_trace, Item p_from_item) {
        ...
        if (p_from_item instanceof net.freerouting.freeroute.board.DrillItem) {
            Point from_center = ((net.freerouting.freeroute.board.DrillItem) p_from_item).get_center();
            points_matching = from_center.equals(p_trace.first_corner()) || from_center.equals(p_trace.last_corner());
        } else if (p_from_item instanceof net.freerouting.freeroute.board.PolylineTrace) {
            net.freerouting.freeroute.board.PolylineTrace from_trace = (net.freerouting.freeroute.board.PolylineTrace) p_from_item;
    ...
    public static class DoorSection {
        ...
        private DoorSection(ExpansionDoor p_door, int p_section_no, FloatLine p_section_line) {
            door = p_door;
