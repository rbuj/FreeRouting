...
import net.freerouting.freeroute.interactive.InteractiveActionThread;
...
public class BatchFanout {
    ...
    private BatchFanout(InteractiveActionThread p_thread) {
        ...
        Collection<net.freerouting.freeroute.board.Pin> board_smd_pin_list = routing_board.get_smd_pins();
        this.sorted_components = new java.util.TreeSet<>();
        for (int i = 1; i <= routing_board.components.count(); ++i) {
            net.freerouting.freeroute.board.Component curr_board_component = routing_board.components.get(i);
    ...
    private int fanout_pass(int p_pass_no) {
        ...
        if (this.routing_board.get_test_level() != net.freerouting.freeroute.board.TestLevel.RELEASE_VERSION) {
    ...
    private static class Component implements Comparable<Component> {
        ...
        final net.freerouting.freeroute.board.Component board_component;
        ...
        Component(net.freerouting.freeroute.board.Component p_board_component, Collection<net.freerouting.freeroute.board.Pin> p_board_smd_pin_list) {
            this.board_component = p_board_component;

            // calcoulate the center of gravity of all SMD pins of this component.
            Collection<net.freerouting.freeroute.board.Pin> curr_pin_list = new java.util.LinkedList<>();
            int cmp_no = p_board_component.no;
            for (net.freerouting.freeroute.board.Pin curr_board_pin : p_board_smd_pin_list) {
            ...
            double x = 0;
            double y = 0;
            for (net.freerouting.freeroute.board.Pin curr_pin : curr_pin_list) {
            ...
            for (net.freerouting.freeroute.board.Pin curr_board_pin : curr_pin_list) {
        ...
        class Pin implements Comparable<Pin> {
            ...
            final net.freerouting.freeroute.board.Pin board_pin;
            final double distance_to_component_center;

            Pin(net.freerouting.freeroute.board.Pin p_board_pin) {
