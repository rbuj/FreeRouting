...
public final class WindowRouteParameter extends BoardSavableSubWindow {
    ...
    private WindowRouteParameter(BoardFrame p_board_frame) {
        ...
        if (this.board_handling.get_routing_board().get_test_level() != net.freerouting.freeroute.board.TestLevel.RELEASE_VERSION) {
    ...
    private class SnapAngle90Listener implements java.awt.event.ActionListener {
        ...
        public void actionPerformed(java.awt.event.ActionEvent p_evt) {
            if (board_handling.get_routing_board().rules.get_trace_angle_restriction() == net.freerouting.freeroute.board.AngleRestriction.NINETY_DEGREE) {
                return;
            }
            Collection<net.freerouting.freeroute.board.Trace> trace_list = board_handling.get_routing_board().get_traces();
            boolean free_angle_traces_found = false;
            for (net.freerouting.freeroute.board.Trace curr_trace : trace_list) {
                if (curr_trace instanceof net.freerouting.freeroute.board.PolylineTrace) {
                    if (!((net.freerouting.freeroute.board.PolylineTrace) curr_trace).polyline().is_orthogonal()) {
            ...
            board_handling.set_current_snap_angle(net.freerouting.freeroute.board.AngleRestriction.NINETY_DEGREE);
        }
    }
    ...
    private class SnapAngle45Listener implements java.awt.event.ActionListener {
        ...
        @Override
        public void actionPerformed(java.awt.event.ActionEvent p_evt) {
            if (board_handling.get_routing_board().rules.get_trace_angle_restriction() == net.freerouting.freeroute.board.AngleRestriction.FORTYFIVE_DEGREE) {
                return;
            }
            Collection<net.freerouting.freeroute.board.Trace> trace_list = board_handling.get_routing_board().get_traces();
            boolean free_angle_traces_found = false;
            for (net.freerouting.freeroute.board.Trace curr_trace : trace_list) {
                if (curr_trace instanceof net.freerouting.freeroute.board.PolylineTrace) {
                    if (!((net.freerouting.freeroute.board.PolylineTrace) curr_trace).polyline().is_multiple_of_45_degree()) {
                        free_angle_traces_found = true;
                        break;
                    }
                }
            ...
            board_handling.set_current_snap_angle(net.freerouting.freeroute.board.AngleRestriction.FORTYFIVE_DEGREE);
        }
    }
    ...
    private class SnapAngleNoneListener implements java.awt.event.ActionListener {
        ...
        @Override
        public void actionPerformed(java.awt.event.ActionEvent p_evt) {
            board_handling.set_current_snap_angle(net.freerouting.freeroute.board.AngleRestriction.NONE);
        }
    }
