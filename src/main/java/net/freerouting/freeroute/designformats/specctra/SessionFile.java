...
public class SessionFile {
    ...
    public static void write_components(BasicBoard p_board, IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform,
        ...
        for (net.freerouting.freeroute.board.Component curr_component : p_board.components) {
            if (curr_component.get_package() == p_package) {
                ...
                Iterator<net.freerouting.freeroute.board.Item> it = board_items.iterator();
                while (it.hasNext()) {
                    net.freerouting.freeroute.board.Item curr_item = it.next();
    ...
    public static void write_component(BasicBoard p_board, IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform,
            IndentFileWriter p_file, net.freerouting.freeroute.board.Component p_component) throws java.io.IOException {
        p_file.new_line();
        p_file.write("(place ");
    ...
    public static void write_was_is(BasicBoard p_board, IdentifierType p_identifier_type,
        ...
        for (net.freerouting.freeroute.board.Pin curr_pin : board_pins) {
            ...
            if (curr_pin.get_changed_to() != curr_pin) {
                p_file.new_line();
                p_file.write("(pins ");
                net.freerouting.freeroute.board.Component curr_cmp = p_board.components.get(curr_pin.get_component_no());
                if (curr_cmp != null) {
                    p_identifier_type.write(curr_cmp.name, p_file);
                    p_file.write("-");
                    net.freerouting.freeroute.library.Package.Pin package_pin = curr_cmp.get_package().get_pin(curr_pin.get_index_in_package());
                    p_identifier_type.write(package_pin.name, p_file);
                } else {
                    System.out.println("SessionFile.write_was_is: component not found");
                }
                p_file.write(" ");
                net.freerouting.freeroute.board.Component swap_cmp = p_board.components.get(swapped_with.get_component_no());
                if (swap_cmp != null) {
                    p_identifier_type.write(swap_cmp.name, p_file);
                    p_file.write("-");
                    net.freerouting.freeroute.library.Package.Pin package_pin = swap_cmp.get_package().get_pin(swapped_with.get_index_in_package());
                    p_identifier_type.write(package_pin.name, p_file);
                } else {
                    System.out.println("SessionFile.write_was_is: component not found");
                }
    ...
    private static void write_padstack(net.freerouting.freeroute.library.Padstack p_padstack, BasicBoard p_board, IdentifierType p_identifier_type,
    ...
    private static void write_net(int p_net_no, BasicBoard p_board, IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform,
            IndentFileWriter p_file) throws java.io.IOException {
        Collection<net.freerouting.freeroute.board.Item> net_items = p_board.get_connectable_items(p_net_no);
        boolean header_written = false;
        Iterator<net.freerouting.freeroute.board.Item> it = net_items.iterator();
        while (it.hasNext()) {
            net.freerouting.freeroute.board.Item curr_item = it.next();
            if (curr_item.get_fixed_state() == net.freerouting.freeroute.board.FixedState.SYSTEM_FIXED) {
                continue;
            }
    ...
    static private void write_fixed_state(IndentFileWriter p_file, net.freerouting.freeroute.board.FixedState p_fixed_state) throws java.io.IOException {
        ...
        p_file.new_line();
        p_file.write("(type ");
        if (p_fixed_state == net.freerouting.freeroute.board.FixedState.SYSTEM_FIXED) {
            p_file.write("fix)");
        } else {
