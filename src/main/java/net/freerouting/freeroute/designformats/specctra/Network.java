...
public class Network extends ScopeKeyword {
    ...
    private static void write_circuit(net.freerouting.freeroute.rules.NetClass p_net_class, WriteScopeParameter p_par)
            throws java.io.IOException {
    ...
    private static void insert_via_infos(Collection<net.freerouting.freeroute.rules.ViaInfo> p_via_infos, RoutingBoard p_board, boolean p_attach_allowed) {
        if (p_via_infos.size() > 0) {
            for (net.freerouting.freeroute.rules.ViaInfo curr_info : p_via_infos) {
    ...
    private static void create_default_via_infos(net.freerouting.freeroute.board.BasicBoard p_board, net.freerouting.freeroute.rules.NetClass p_net_class, boolean p_attach_allowed) {
        ...
        for (int i = 0; i < p_board.library.via_padstack_count(); ++i) {
            ...
            net.freerouting.freeroute.rules.ViaInfo found_via_info
                    = new net.freerouting.freeroute.rules.ViaInfo(via_name, curr_padstack, cl_class, attach_allowed, p_board.rules);
            p_board.rules.via_infos.add(found_via_info);
        }
    }

    private static void insert_via_rules(Collection<Collection<String>> p_via_rules, net.freerouting.freeroute.board.BasicBoard p_board) {
     ...
     * with the same
     */
    static boolean add_via_rule(Collection<String> p_name_list, net.freerouting.freeroute.board.BasicBoard p_board) {
        Iterator<String> it = p_name_list.iterator();
        String rule_name = it.next();
        net.freerouting.freeroute.rules.ViaRule existing_rule = p_board.rules.get_via_rule(rule_name);
        net.freerouting.freeroute.rules.ViaRule curr_rule = new net.freerouting.freeroute.rules.ViaRule(rule_name);
        boolean rule_ok = true;
        while (it.hasNext()) {
            net.freerouting.freeroute.rules.ViaInfo curr_via = p_board.rules.via_infos.get(it.next());
            if (curr_via != null) {
    ...
    static void insert_net_class(NetClass p_class, LayerStructure p_layer_structure, net.freerouting.freeroute.board.BasicBoard p_board, CoordinateTransform p_coordinate_transform,
            boolean p_via_at_smd_allowed) {
        net.freerouting.freeroute.rules.NetClass board_net_class = p_board.rules.append_net_class(p_class.name);
        ...
        if (p_class.via_rule != null) {
            net.freerouting.freeroute.rules.ViaRule via_rule = p_board.rules.get_via_rule(p_class.via_rule);
            if (via_rule != null) {
        ...
        for (String curr_net_name : p_class.net_list) {
            Collection<net.freerouting.freeroute.rules.Net> curr_net_list = p_board.rules.nets.get(curr_net_name);
            for (net.freerouting.freeroute.rules.Net curr_net : curr_net_list) {
    ...
    static private void insert_class_pairs(Collection<NetClass.ClassClass> p_class_classes, ReadScopeParameter p_par) {
        for (NetClass.ClassClass curr_class_class : p_class_classes) {
            ...
            while (it1.hasNext()) {
                ...
                net.freerouting.freeroute.rules.NetClass first_class = routing_board.rules.net_classes.get(first_name);
                if (first_class == null) {
                    ...
                    while (it2.hasNext()) {
                        ...
                        net.freerouting.freeroute.rules.NetClass second_class = routing_board.rules.net_classes.get(second_name);
    ...
    private static void create_active_trace_layers(Collection<String> p_use_layer, LayerStructure p_layer_structure, net.freerouting.freeroute.rules.NetClass p_net_class) {
    ...
    private static void add_clearance_rule(net.freerouting.freeroute.rules.ClearanceMatrix p_clearance_matrix, net.freerouting.freeroute.rules.NetClass p_net_class,
    ...
    private static boolean insert_logical_parts(ReadScopeParameter p_par) {
        ...
        for (PartLibrary.LogicalPart next_part : p_par.logical_parts) {
            net.freerouting.freeroute.library.Package lib_package = search_lib_package(next_part.name, p_par.logical_part_mappings, routing_board);
            if (lib_package == null) {
                return false;
            }
        ...
        for (PartLibrary.LogicalPartMapping next_mapping : p_par.logical_part_mappings) {
            net.freerouting.freeroute.library.LogicalPart curr_logical_part = routing_board.library.logical_parts.get(next_mapping.name);
            {
            ...
            for (String curr_cmp_name : next_mapping.components) {
                net.freerouting.freeroute.board.Component curr_component = routing_board.components.get(curr_cmp_name);
     ...
     */
    private static net.freerouting.freeroute.library.Package search_lib_package(String p_part_name,
            java.util.Collection<PartLibrary.LogicalPartMapping> p_logical_part_mappings, net.freerouting.freeroute.board.BasicBoard p_board) {
        for (PartLibrary.LogicalPartMapping curr_mapping : p_logical_part_mappings) {
            if (curr_mapping.name.equals(p_part_name)) {
                ...
                net.freerouting.freeroute.board.Component curr_component = p_board.components.get(component_name);
                if (curr_component == null) {
    ...
    private static void insert_component(ComponentPlacement.ComponentLocation p_location, String p_lib_key,
        ...
        net.freerouting.freeroute.board.RoutingBoard routing_board = p_par.board_handling.get_routing_board();
        net.freerouting.freeroute.library.Package curr_front_package = routing_board.library.packages.get(p_lib_key, true);
        net.freerouting.freeroute.library.Package curr_back_package = routing_board.library.packages.get(p_lib_key, false);
        ...
        net.freerouting.freeroute.board.Component new_component = routing_board.components.add(p_location.name, component_location,
        ...
        if (p_location.position_fixed) {
            ...
            fixed_state = net.freerouting.freeroute.board.FixedState.UNFIXED;
        }
        net.freerouting.freeroute.library.Package curr_package = new_component.get_package();
        for (int i = 0; i < curr_package.pin_count(); ++i) {
            net.freerouting.freeroute.library.Package.Pin curr_pin = curr_package.get_pin(i);
            net.freerouting.freeroute.library.Padstack curr_padstack = routing_board.library.padstacks.get(curr_pin.padstack_no);
            ...
            for (Net curr_pin_net : pin_nets) {
                net.freerouting.freeroute.rules.Net curr_board_net = routing_board.rules.nets.get(curr_pin_net.id.name, curr_pin_net.id.subnet_number);
            ...
            net.freerouting.freeroute.rules.NetClass net_class;
            net.freerouting.freeroute.rules.Net board_net;
        ...
        for (int k = 0; k <= 2; ++k) {
            net.freerouting.freeroute.library.Package.Keepout[] keepout_arr;
            ...
            if (!net_rules.isEmpty()) {
                ...
                while (it.hasNext()) {
                    ...
                    if (curr_ob instanceof Rule.WidthRule) {
                        net.freerouting.freeroute.rules.NetClass default_net_rule = p_board.rules.get_default_net_class();
                        ...
                        net.freerouting.freeroute.rules.NetClass net_rule
