...
public class SessionToEagle extends javax.swing.JFrame {
    ...
    public static boolean get_instance(java.io.InputStream p_session, java.io.OutputStream p_output_stream,
        ...
        Scanner scanner = new SpecctraFileScanner(new InputStreamReader(p_session));
     ...
     */
    private final net.freerouting.freeroute.board.BasicBoard board;
    /**
     * The layer structure in specctra format
     */
    private final LayerStructure specctra_layer_structure;
    private final net.freerouting.freeroute.board.Unit unit;
    /**
    ...
    private boolean process_session_scope() throws java.io.IOException {
        ...
        net.freerouting.freeroute.geometry.planar.IntBox board_bounding_box = this.board.get_bounding_box();
    ...
    private boolean process_wire_scope(String p_net_name) throws java.io.IOException {
        ...
        for (int i = 0; i < wire_path.coordinate_arr.length; ++i) {
            ...
            if (i % 2 == 0) {
                this.out_file.write(" ");
            } else if (i == wire_path.coordinate_arr.length - 1) {
                this.out_file.write(")");
            } else {
                this.out_file.write(") (");
            }
    ...
    private boolean process_via_scope(String p_net_name) throws java.io.IOException {
        ...
        net.freerouting.freeroute.geometry.planar.ConvexShape via_shape = via_padstack.get_shape(via_padstack.from_layer());
        ...
        if (via_shape instanceof net.freerouting.freeroute.geometry.planar.Circle) {
            this.out_file.write(" round ");
        } else if (via_shape instanceof net.freerouting.freeroute.geometry.planar.IntOctagon) {
    ...
    private boolean process_swapped_pins(int p_component_no) throws java.io.IOException {
        java.util.Collection<net.freerouting.freeroute.board.Pin> component_pins = this.board.get_component_pins(p_component_no);
        boolean component_has_swapped_pins = false;
        for (net.freerouting.freeroute.board.Pin curr_pin : component_pins) {
    ...
    private static class PinInfo {
        ...
        net.freerouting.freeroute.board.Pin curr_changed_to;

        PinInfo(net.freerouting.freeroute.board.Pin p_pin) {
            pin = p_pin;
