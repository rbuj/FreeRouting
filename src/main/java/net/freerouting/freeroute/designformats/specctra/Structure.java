...
class Structure extends ScopeKeyword {
    ...
    static void write_scope(WriteScopeParameter p_par) throws java.io.IOException, WriteScopeException {
        ...
        for (;;) {
            ...
            if (curr_ob instanceof net.freerouting.freeroute.board.BoardOutline) {
                break;
            }
        }
        if (curr_ob == null) {
        ...
        for (;;) {
            ...
            if (!(curr_ob instanceof net.freerouting.freeroute.board.ObstacleArea)) {
                continue;
            }
            net.freerouting.freeroute.board.ObstacleArea curr_keepout = (net.freerouting.freeroute.board.ObstacleArea) curr_ob;
            if (curr_keepout.get_component_no() != 0) {
                // keepouts belonging to a component are not written individually.
                continue;
            }
            if (curr_keepout instanceof net.freerouting.freeroute.board.ConductionArea) {
        ...
        for (;;) {
            ...
            Plane.write_scope(p_par, (net.freerouting.freeroute.board.ConductionArea) curr_ob);
        }
        p_par.file.end_scope();
    }
    ...
    private static void write_via_padstacks(net.freerouting.freeroute.library.BoardLibrary p_library, IndentFileWriter p_file,
        ...
        for (int i = 0; i < p_library.via_padstack_count(); ++i) {
            net.freerouting.freeroute.library.Padstack curr_padstack = p_library.get_via_padstack(i);
    ...
    private static void write_keepout_scope(WriteScopeParameter p_par, net.freerouting.freeroute.board.ObstacleArea p_keepout) throws java.io.IOException {
        ...
        if (keepout_area instanceof net.freerouting.freeroute.geometry.planar.Shape) {
            boundary_shape = (net.freerouting.freeroute.geometry.planar.Shape) keepout_area;
    ...
    private static void insert_missing_power_planes(Collection<LayerInfo> p_layer_info,
        ...
        for (LayerInfo curr_layer : p_layer_info) {
            ...
            if (!conduction_area_found && !curr_layer.net_names.isEmpty()) {
                ...
                net.freerouting.freeroute.rules.Net curr_net = p_board.rules.nets.get(curr_net_id.name, curr_net_id.subnet_number);
                {
                    if (curr_net == null) {
     ...
     */
    private static boolean clearance_equals(net.freerouting.freeroute.rules.ClearanceMatrix p_cl_matrix, int p_layer_1, int p_layer_2) {
    ...
    private static boolean insert_keepout(Area p_area, ReadScopeParameter p_par,
        ...
        net.freerouting.freeroute.board.BasicBoard board = p_par.board_handling.get_routing_board();
        if (board == null) {
    ...
    private static void insert_keepout(net.freerouting.freeroute.board.BasicBoard p_board, net.freerouting.freeroute.geometry.planar.Area p_area, int p_layer,
    ...
    boolean read_scope(ReadScopeParameter p_par) {
        ...
        for (;;) {
            ...
            if (prev_token == Keyword.OPEN_BRACKET) {
                ...
                } else if (next_token == PLANE_SCOPE) {
                    if (p_par.layer_structure == null) {
                        p_par.layer_structure = new LayerStructure(board_construction_info.layer_info);
                    }
        ...
        while (it.hasNext()) {
            ...
            net.freerouting.freeroute.rules.Net curr_net = board.rules.nets.get(plane_info.net_name, 1);
            if (curr_net == null) {
    ...
    private boolean create_board(ReadScopeParameter p_par, BoardConstructionInfo p_board_construction_info) {
        ...
        while (5 * max_coor >= net.freerouting.freeroute.geometry.planar.Limits.CRIT_INT) {
            scale_factor /= 10;
            max_coor /= 10;
        }
        ...
        net.freerouting.freeroute.rules.ClearanceMatrix clearance_matrix = net.freerouting.freeroute.rules.ClearanceMatrix.get_default_instance(board_layer_structure, 0);
        net.freerouting.freeroute.rules.BoardRules board_rules = new net.freerouting.freeroute.rules.BoardRules(board_layer_structure, clearance_matrix);
        net.freerouting.freeroute.board.Communication.SpecctraParserInfo specctra_parser_info
                = new net.freerouting.freeroute.board.Communication.SpecctraParserInfo(p_par.string_quote, p_par.host_cad,
                        p_par.host_version, p_par.constants, p_par.write_resolution, p_par.dsn_file_generated_by_host);
        net.freerouting.freeroute.board.Communication board_communication
                = new net.freerouting.freeroute.board.Communication(p_par.unit, p_par.resolution, specctra_parser_info,
