...
public class Component extends ScopeKeyword {
    ...
    private static void write_keepout_infos(WriteScopeParameter p_par, net.freerouting.freeroute.board.Component p_component)
        ...
        net.freerouting.freeroute.library.Package.Keepout[] curr_keepout_arr;
        String keepout_type;
        for (int j = 0; j < 3; ++j) {
            switch (j) {
                case 0:
            ...
            for (int i = 0; i < curr_keepout_arr.length; ++i) {
                net.freerouting.freeroute.library.Package.Keepout curr_keepout = curr_keepout_arr[i];
                net.freerouting.freeroute.board.ObstacleArea curr_obstacle_area = get_keepout(p_par.board, p_component.no, curr_keepout.name);
    ...
    private static net.freerouting.freeroute.board.ObstacleArea get_keepout(net.freerouting.freeroute.board.BasicBoard p_board, int p_component_no, String p_name) {
        ...
        for (;;) {
            net.freerouting.freeroute.board.Item curr_item = (net.freerouting.freeroute.board.Item) p_board.item_list.read_object(it);
            if (curr_item == null) {
                break;
            }
            if (curr_item.get_component_no() == p_component_no && curr_item instanceof net.freerouting.freeroute.board.ObstacleArea) {
                net.freerouting.freeroute.board.ObstacleArea curr_area = (net.freerouting.freeroute.board.ObstacleArea) curr_item;
