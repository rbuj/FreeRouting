...
public class Net implements Comparable<Net>, net.freerouting.freeroute.board.ObjectInfoPanel.Printable, java.io.Serializable {
    ...
    public java.util.Collection<Item> get_terminal_items() {
        ...
        net.freerouting.freeroute.board.BasicBoard board = this.net_list.get_board();
        ...
        for (;;) {
            ...
            if (curr_item instanceof net.freerouting.freeroute.board.Connectable) {
     ...
     */
    public java.util.Collection<net.freerouting.freeroute.board.Pin> get_pins() {
        java.util.Collection<net.freerouting.freeroute.board.Pin> result = new java.util.LinkedList<>();
        net.freerouting.freeroute.board.BasicBoard board = this.net_list.get_board();
        java.util.Iterator<UndoableObjects.UndoableObjectNode> it = board.item_list.start_read_object();
        for (;;) {
            Item curr_item = (Item) board.item_list.read_object(it);
            ...
            if (curr_item instanceof net.freerouting.freeroute.board.Pin) {
                if (curr_item.contains_net(this.net_number)) {
                    result.add((net.freerouting.freeroute.board.Pin) curr_item);
                }
            }
        }
        return result;
     ...
     */
    public java.util.Collection<net.freerouting.freeroute.board.Item> get_items() {
        java.util.Collection<net.freerouting.freeroute.board.Item> result = new java.util.LinkedList<>();
        net.freerouting.freeroute.board.BasicBoard board = this.net_list.get_board();
        java.util.Iterator<UndoableObjects.UndoableObjectNode> it = board.item_list.start_read_object();
        for (;;) {
            Item curr_item = (Item) board.item_list.read_object(it);
    ...
    public double get_trace_length() {
        ...
        for (Item curr_item : net_items) {
            ...
            if (curr_item instanceof net.freerouting.freeroute.board.Trace) {
                cumulative_trace_length += ((net.freerouting.freeroute.board.Trace) curr_item).get_length();
            }
        }
